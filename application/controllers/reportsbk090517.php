        ...
        if($this->session->userdata(SESSION_NAME)) :
            if ($this->profile_level() != 5 && $this->profile_level() != 6 && $this->profile_level() != 7 && $this->profile_level() != 8 && $this->profile_level() != 9) :
                ...
                foreach ($pend_data as $row) :
                    ...
                    $csvdata = $row->quantity.', '.$pend_dataitem['pi_unit'].', '.$pend_dataitem['item_name'];
                    fputcsv($file, explode(',' ,$csvdata));
                endforeach;
                            ...
                            foreach ($consumpt_dept as $rowdept) :
                                ...
                                if ($consumpt_item) :
                                    foreach ($consumpt_item as $rowitem) :
                                        ...
                                        $consumptdata .= ', '.$rowitem->qty_total.' (Php '.number_format($consumptprice, 2, '.', '').')';
                                    endforeach;
                                else :
                                    $consumptdata .= ', 0 (Php 0.00)';
                                endif;                                
                            endforeach;
                            if ($consumpt_total_item) :
                                foreach ($consumpt_total_item as $total_item) :  
                                    $consumptprice = $total_item->qty_total * $row->item_price;               
                                    $consumptdata .= ','.$total_item->qty_total.' (Php '.number_format($consumptprice, 2, '.', '').')';
                                endforeach;
                            else :
                                $consumptdata .= ', 0 (Php 0.00)';
                            endif;
                    ...
                    foreach ($consumpt_fdept as $rowfdept) :                    
                        ...
                        if ($consumpt_total_dept) :
                            foreach ($consumpt_total_dept as $total_dept) :                                
                                ...
                                $consumptfoot .= ','.$total_dept->qty_total.' (Php '.number_format($consumptprice, 2, '.', '').')';
                            endforeach;
                        else :
                            $consumptfoot .= ', 0 (Php 0.00)';
                        endif;
                    endforeach; 
    ...
    public function csv_dailyapprove()
	{
        if($this->session->userdata(SESSION_NAME)) :
            if ($this->profile_level() != 5 && $this->profile_level() != 6 && $this->profile_level() != 7 && $this->profile_level() != 8 && $this->profile_level() != 9) :
                ...
                $csvhead = 'Daily Approved Request Report from 12:00am to 11:59pm '.$fromto;
                fputcsv($file, explode(',' ,$csvhead));        
                ...
                if ($consumpt_htrans) :
                    ...
                    foreach ($consumpt_htrans as $rowhtrans) :                    
                        $consumpthead .= ', '.mdate('%Y', strtotime($rowhtrans->trans_date)).'-'.$rowhtrans->trans_date.' ('.$rowhtrans->user_empnum.' - '.$rowhtrans->dept_abbr.')';
                    endforeach;        
                    $csvheader = "Item".$consumpthead.", Total";
                    fputcsv($file, explode(',' ,$csvheader));        
